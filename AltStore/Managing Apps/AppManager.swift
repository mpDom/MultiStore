...
final class AppManager: ObservableObject, @unchecked Sendable
{
                ...
                completionHandler: @escaping (Result<InstalledApp, Error>) -> Void) -> Progress
    {
        ...
        // (the operation carries the new AppVersion, so this can't be inferred from the operation).
        context.accountID = Self.inferredAccountID(for: [.refresh(installedApp)])
    ...
    func refresh(_ installedApps: [InstalledApp], presentingViewController: UIViewController?, group: RefreshGroup? = nil) -> RefreshGroup
    {
        ...
        actualGroup.activeTask = Task.detached {
            ...
            await self.refresh(partitions: partitions, handler: pipelineHandler, presentingViewController: presentingViewController, aggregateGroup: actualGroup)
        }
...
extension AppManager: PipelineProgress, PipelineExecutionContext, PipelineErrorLogger {
    ...
    func makeAuthenticatedContext(presentingViewController: UIViewController?,
