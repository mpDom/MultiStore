...
final class FetchAnisetteDataOperation: BaseStandaloneOperation<AuthenticatedOperationContext, ALTAnisetteData>, @unchecked Sendable {
    // Since early September 2026 Apple's GrandSlam edge answers HTTP 503 (an HTML page, which then
    // fails plist parsing as NSCocoaErrorDomain 3840) to any request whose X-MMe-Client-Info contains
    // "com.apple.dt.Xcode". Identify as akd instead (upstream SideStore/AnisetteKit@ebab7b9).
    ...
    var resolvedClientInfo: String {
        Self.sanitizedClientInfo(self.activeConfig?.clientInfo ?? self.clientInfo ?? FetchAnisetteDataOperation.defaultClientInfo)
    }

    /// Rewrites a stored/served client-info that still carries the Xcode identity Apple now rejects
    /// (see `defaultClientInfo`) to the akd identity. Mirrors omnisette's change in
    /// SideStore/apple-private-apis#27 so old config files and unpatched servers keep working.
