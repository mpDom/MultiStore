...
final class ResignAppOperation: BasePipelineOperation<InstallAppOperationContext, ALTApplication>, @unchecked Sendable {
    ...
    private func prepareAppBundle(for targetAppBundle: ALTApplication, profiles: [String: ALTProvisioningProfile], appexBundleIds: [String: String]) async throws -> URL {
        ...
        if targetAppBundle.isAltStoreApp {
            ...
            // falling back to the global active certificate for backwards compatibility.
            if let contextCertificate = self.context.overrideCertificate ?? self.context.authenticatedContext.signingCertificate {
                let password = CertificateManager.shared.getPassword(for: contextCertificate)
                let p12Data = try CertificateManager.convert(contextCertificate, password: password)
                additionalValues[Bundle.Info.certificateID] = contextCertificate.serialNumber
                try p12Data.write(to: appBundle.certificateURL, options: .atomic)
            } else if let activeCert = CertificateManager.shared.activeCertificate {
