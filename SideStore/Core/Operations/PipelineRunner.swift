...
final class PipelineRunner: Sendable
{
                 ...
                 group: RefreshGroup) async throws -> RefreshGroup
    {
        ...
        try await Task.detached {
            ...
            {
                // Multi-account: if no specific account was requested, infer the signing account
                // from the operations' apps so single-app actions (resign / refresh / activate / ...)
                // authenticate with -- and re-sign using -- the account that actually signs the app
                // rather than the default account.
                if group.context.accountID == nil
                {
                    group.context.accountID = AppManager.inferredAccountID(for: operations)
                }
