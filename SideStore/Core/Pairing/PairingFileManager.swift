...
extension PairingFileManager: UIDocumentPickerDelegate {
    ...
    func presentPairingFileAlert(on vc: UIViewController, isRetry: Bool, completion: ((URL?) -> Void)? = nil) {
        ...
        alert.addAction(UIAlertAction(title: NSLocalizedString("Select File", comment: ""), style: .default) { _ in
            ...
            // (.plist, .mobiledevicepairing, none, …) and iOS sometimes tags a plain .plist in a way the
            // narrow type list doesn't match, greying it out. The chosen file is validated when minimuxer
            // starts, so a wrong pick fails gracefully instead of blocking selection.
            types.append(contentsOf: [.propertyList, .text, .data, .item])
